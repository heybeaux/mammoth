...
P3 callers may attach validated Temporal execution metadata: stable workflow ID,
ordered run chain, task queue, durable step, attempts, operation timeline,
metrics, and structured logs. Those links are included in the deterministic
digest, must point at the linked run chain and an existing or prior authoritative
revision, and remain diagnostic; they cannot add claims, evidence, audit events,
or dossier sentences.
