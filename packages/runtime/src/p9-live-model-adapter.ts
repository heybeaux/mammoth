  ...
  async synthesizeReport(request: {
    ...
    const outcome = await this.#complete(
      ...
      [
        ...
        'Answer the research question directly. The first_bounded_change lead must contrast the admitted current state with exactly one proposed implementation delta, use add, introduce, replace, remove, modify, or implement, name a concrete flag, harness, benchmark, test, function, kernel, configuration, mode, or check, and explain why it comes first. Do not use vague phrases such as verification path or optimization path. Never recommend behavior that the admitted current-state evidence says is already implemented.',
